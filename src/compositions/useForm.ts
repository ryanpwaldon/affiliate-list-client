...
const validateAll = async (fields: Record<string, Field>) => {
  ...
  const errors = fieldValues.map(field => field.error)
  return errors.every(value => !value)
}

export default <T extends Record<keyof T, AnySchema>>(schema: ComputedRef<ObjectSchema<T>>) => {
  const fields = ref() as Ref<Record<keyof T, Field>>
  watch(schema, () => (fields.value = createFields(schema.value)), { immediate: true })
  const modified = computed(() => !(Object.values(fields.value) as Field[]).map(({ modified }) => modified).every(value => !value))
  ...
  return { fields, modified, values, handleSubmit }
}
